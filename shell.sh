...
do
	...
	echo ""
		...
		echo ""
		echo ""
		echo -n "Enter the IP Address for which you need corresponding details : "
		read ip
...
#SENT packages
		...
		echo  "List of available source domain names :- "
		echo ""		
		uniq TraceToWork12.txt
		echo -e "\n"		
		echo -n "Enter the source domain name : "
		read dName
		...
		h1=`wc -l < TraceToWork25.txt`
			...
			do
			echo "Invalid"
			echo -n "Enter the source domain name again - "
			read dName
			awk '$0=="'"$dName"'"' TraceToWork12.txt > TraceToWork25.txt 
			h1=`wc -l < TraceToWork25.txt`
			done
		...
		echo -e "    S.No  Time\t\tSource Domain Name\t    Destination IP     Protocol    Length    Information  "		
...
#RECEIVED packages
		...
		sort -u TraceToWork11.txt > TraceToWork12.txt
		echo -e "------------------------------------------------------------------------------------------------------------\n"
		echo "The available Destination domain names are:- "
		echo ""
		uniq TraceToWork12.txt
		echo ""		
		echo -n "Enter the Destination domain name :- "
		read dName
		awk '$0=="'"$dName"'"' TraceToWork12.txt > TraceToWork25.txt 
		h2=`wc -l < TraceToWork25.txt`
			...
			do
			echo "Invalid"
			echo -n "Enter the Destination Domain name again :- "
			read dName 
			h2=`wc -l < TraceToWork25.txt`
			done
		echo -e "The RECEIVED packets corresponding to the destination domain name are- \n\n"
		...
		4) 
		echo ""
		echo -n "Enter the captured filename for Statistics  : "
		read fileName
		echo -e "\n\n\n--------------------------------------------------------STATISTICS FOR THE CAPTURED 					FILE------------------------------------------------------------\n\n"
		capinfos -A $fileName
		echo -e "\n\n----------------------------------------------------------------THANK YOU!!-------------------------------------------------------------------"
		exit 0
		;;
